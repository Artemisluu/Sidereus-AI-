...
export default function App() {
  ...
  const compareItems = useMemo(
    () => (candidatesQuery.data ?? []).filter((candidate) => compareIds.includes(candidate.id)).slice(0, 3),
    [candidatesQuery.data, compareIds],
  );

  const selectedCandidate = candidateQuery.data ?? null;
  const selectedCandidateName =
    selectedCandidate?.structuredData?.basicInfo?.name || selectedCandidate?.filename || '未选择';
  const summaryItems = [
    {
      label: '候选人总数',
    ...
    <main className='ops-shell'>
      <header className='ops-hero'>
        <div className='relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between'>
          ...
          <button type='button' className='control-button self-start' onClick={toggleTheme}>
            主题切换（Ctrl+K）
          </button>
        </div>
      ...
      {globalError && <div className='ops-alert mt-5'>{globalError}</div>}

      <div className='mt-6 grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)] 2xl:grid-cols-[320px_minmax(0,1fr)]'>
      ...
      <section className='space-y-5 xl:col-span-2 2xl:col-span-1'>
        <div className='flex items-end justify-between gap-3 px-1'>
          <div>
            <span className='ops-section-label'>详情工作台</span>
            <p className='ops-copy mt-2 text-sm'>查看结构化结果、提取进度、评分细节和原始 PDF。</p>
          </div>
          <span className='ops-section-note'>{selectedCandidateId ? '已锁定当前候选人' : '请选择候选人'}</span>
        </div>
        ...
        <div className='mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3'>
          {compareItems.map((candidate) => (
            <article key={candidate.id} className='compare-card'>
              ...
              <span className='ops-status-pill mt-3 inline-flex'>{candidate.status}</span>
            </article>
          ))}
          {!compareItems.length && <p className='compare-empty-state'>在候选人列表中勾选 2-3 人即可对比。</p>}
        </div>
      </section>
    </main>
  );
